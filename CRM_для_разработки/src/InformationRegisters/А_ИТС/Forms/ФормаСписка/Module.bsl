...
Функция ПолучитьВидПодписки(ВидИТС, КодВидаИТС, Отказ, МассивКодовСОшибкой)
	...
	КодВидаИТС = Формат(Число(КодВидаИТС), "ЧГ=;");
...
Процедура ЗагрузитьДанныеНаСервере(Адрес, РасширениеФайла)	
	...
	ДатаОтчета = НачалоМесяца(ТекущаяДатаСеанса());
	...
	Для Каждого СтрокаФайла Из ХЛС Цикл
		...
		ВидПодпискиИТС = ПолучитьВидПодписки(СтрокаФайла.НаименованиеВида1С_ИТС, СтрокаФайла.Вид1С_ИТС, Отказ, МассивКодовСОшибкой);
		...
		Если НЕ Отказ Тогда
			...
			ДатаРегистрации = СтрокаФайла.ДатаРегистрацииДоговора1С_ИТС;
			ДанныеПолей	= СтрокаТЗВСтрокуJSON(ХЛС.Колонки, СтрокаФайла);
			
			//@skip-check query-in-loop
			ЗагрузитьСтрокуОтчетаИТС(ДатаОтчета, РегНомер, ВидПодпискиИТС, СтрокаФайла.ИНН, СтрокаФайла.Пользователь, ДатаНач, ДатаКон, ДатаРегистрации, ДанныеПолей);
...
&НаСервере
Функция СтрокаТЗВСтрокуJSON(КолонкиТЗ, СтрокаТЗ)
...
Процедура ЗагрузитьСтрокуОтчетаИТС(ДатаОтчета, РегНомер, ЕхВидИТС, ЕхИНН, ЕхНаименование, ДатаНач, ДатаКон, ДатаРегистрации, ДанныеПолей)
			...
			Иначе
				...
				Иначе
					...
					НоваяЗапись.Комментарий = "Не найден: " + СокрЛП(Формат(ЕхНаименование) + " ИНН " + ВыбИНН);
